...
namespace Ozon.Route256.Practice.OrdersService.DataAccess.CacheCustomers
{
    public class RedisCustomerRepository : ICacheCustomers
    {
        ...
        public async Task<CustomerFullEntity?> Find(int id, CancellationToken cancellationToken)
        {
            ...
            var result = resultRedis.HasValue ? JsonSerializer.Deserialize<CustomerFullEntity>(resultRedis.ToString(), _jsonSerializerOptions) : null;
            return result;
        }

        public async Task Insert(CustomerFullEntity customer, CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
                return;
