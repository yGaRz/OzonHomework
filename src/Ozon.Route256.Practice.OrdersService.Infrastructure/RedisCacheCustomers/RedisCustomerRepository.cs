...
internal class RedisCustomerRepository : ICacheCustomers
{
    ...
    public async Task<CustomerDal?> Find(int id, CancellationToken cancellationToken)
    {
        ...
        var result = resultRedis.HasValue ? JsonSerializer.Deserialize<CustomerDal>(resultRedis.ToString(), _jsonSerializerOptions) : null;
        return result;
    }

    public async Task Insert(CustomerDal customer, CancellationToken cancellationToken)
    {
        if (cancellationToken.IsCancellationRequested)
            return;
