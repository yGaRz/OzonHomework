...
internal class OrdersShardRepositoryPg : BaseShardRepository, IOrdersRepository
{
    ...
    public async Task Create(OrderDal order, CancellationToken token)
    {
        try
        {
            ...
            values (:id,:customer_id , 
                    ...
                    :order_state::{ShardsHelper.BucketPlaceholder}.order_state_enum, 
                    (CAST(:time_create as timestamp)), 
